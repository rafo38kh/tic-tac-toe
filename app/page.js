    ...
    <div className="flex flex-col items-center p-6 w-full justify-center max-w-[28.75rem]">
      ...
      <div className="flex flex-col items-center justify-center gap-4 bg-semiDarkNavy rounded-lg p-6 w-full my-8 shadow-[0px_-8px_0px_0px_#10212A_inset]">
        ...
        <div className="flex items-center justify-center gap-4 bg-darkNavy rounded-lg w-full p-2 ">
          <button
            className={`${
              playerType === "X" ? "bg-darkSilver" : null
            } w-full rounded-lg p-3 flex items-center justify-center`}
            onClick={() => setPlayerType("X")}
          >
            <svg
            ...
            >
              <path
                ...
                fill={`${playerType === "X" ? "#1A2A33" : "#A8BFC9"}`}
              />
            </svg>
          </button>
          <button
            className={`${
              playerType === "O" ? "bg-darkSilver" : null
            } w-full rounded-lg p-3 flex items-center justify-center`}
            onClick={() => setPlayerType("O")}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="32"
            ...
            >
              <path
                ...
                fill={`${playerType === "O" ? "#1A2A33" : "#A8BFC9"}`}
              />
            </svg>
          </button>
        </div>
      ...
      <div className="flex flex-col w-full">
        <button
          className=" bg-darkYellow text-darkNavy text-base p-3 rounded-lg mb-4 shadow-[0px_-8px_0px_0px_#CC8B13_inset]"
          onClick={() => setGameType("CPU")}
        >
          NEW GAME VS CPU
        </button>
        <Link
          className=" bg-darkBlue text-darkNavy text-base p-3 rounded-lg text-center shadow-[0px_-8px_0px_0px_#118C87_inset]"
          href="/game"
          onClick={() => setGameType("player")}
        >
          NEW GAME VS PLAYER
        </Link>
      </div>
